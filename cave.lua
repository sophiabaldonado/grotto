...
function cave:init()
  ...
  for i = 1, #crystals do
    local light = { health = 1, position = crystals[i], crystal = true }
    light.position[2], light.position[3] = light.position[3], -light.position[2]
    table.insert(self.lights, light)
  end
  for i = 1, #mushrooms do
    local light = { health = 1, position = mushrooms[i], mushroom = true }
    light.position[2], light.position[3] = light.position[3], -light.position[2]
    table.insert(self.lights, light)
  end
end
...
function cave:feel(dt, head, left, right)
  ...
  if self.lights[1] then
    ...
    for i, light in pairs(self.lights) do
      if head:distance(vec3(unpack(light.position))) < .5 then
